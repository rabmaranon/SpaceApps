...
def main():
    ...
    # this is actually the astronaut, but it's quite a hassle to rename everything
    gameBird = Bird()
    ...
    while(gameVariables.waitClick == True):
        ...
        # when the user clicks somewhere, the astronaut will walk/jump and the game will start
        screen.blit(gameImages['space'], (0, 0))
        draw_text(screen, "click to start", 285, 40)
        screen.blit(gameImages['ground'], (0, gameHeight - groundHeight))

        # a new astronaut is 'drawn' every time it moves
        gameBird.redraw(screen, gameImages['astro'], gameImages['astro'])
    ...
    while True:
        ...
        # every time there's a new frame, redraw everything in the environment (ground, astronaut, pipe)
        gameGround.move_and_redraw(screen)

        # update the astronaut position and redrawing it
        ...
        # a collision occurs when the coordinates of the bird overlap with the pipes
        ...
        # the dog went through one half of the pipe's gameWidth; if so, we update the gameScore
        for p in gamePipes:
    ...
    # make the astronaut 'fall' if the round is over
    while(gameBird.bird_y + birdHeight < gameHeight - groundHeight):
        ...
        # make the astronaut fall down and rotates it
        gameBird.redraw_dead(screen, gameImages['astro'])
